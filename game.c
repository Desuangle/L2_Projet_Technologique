...
static int game_index (cgame game, int x, int y){ // fonction auxiliaire, trouver l'index de la pièce actuelle
...
void set_piece_current_dir (game game, int x, int y, direction dir){
	assert(game);
	game->direction[game_index(game,x,y)] = dir;
}
...
piece get_piece(cgame game, int x, int y){
	...
	return game->piece[game_index(game,x,y)];
}
...
direction get_current_dir(cgame game, int x, int y){
	...
	return game->direction[game_index(game,x,y)];
}
